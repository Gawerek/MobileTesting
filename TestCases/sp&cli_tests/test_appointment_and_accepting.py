...
from TestCases.BaseTest import  BaseTest, BaseTestSP
...
@pytest.mark.parametrize("service_info, name, service_type, address, service_names",
                         dataProvider.get_data("Express"))
class Test_BookAppointmentExpress(BaseTestSP):
    def setup_method(self, method):
        var.uuid = None

    def test_book_express_appointment(self, service_info, name, service_type, address, service_names):
        manager = app_manager.AppManager(self.driver)
        home_SP = HomeScreenSP(self.driver)
        profile_SP = home_SP.go_to_profile()
        account_work_mode_SP =profile_SP.click_account_work_mode_button()
        account_work_mode_SP.select_reservation_mode(express=True)
        account_work_mode_SP.click_submit_in_work_mode()
        ...
        news_SP.click_more(0)
        news_SP.verify_status_and_parameters(var.statuses['ACCEPTED'])
        news_SP.click_close_modal_button()
